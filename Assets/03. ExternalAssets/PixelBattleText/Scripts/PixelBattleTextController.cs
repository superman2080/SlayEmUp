...
namespace PixelBattleText
{
	...
	public class PixelBattleTextController : MonoBehaviour
	{
		...
		private Dictionary<TextType, TextAnimation> textDict = new Dictionary<TextType, TextAnimation>();
		...
		private Dictionary<TextType, TextAnimation> InitializeTextDictionary()
        {
			var dict = new Dictionary<TextType, TextAnimation>();
			int length = Enum.GetValues(typeof(TextType)).Length;

            for (int i = 0; i < length; i++)
            {
				var textAnimation = GetTextAnimationByName(Enum.GetName(typeof(TextType), i));
				if (textAnimation != default)
					dict.Add((TextType)i, textAnimation);
            }

			return dict;
        }
		
		private TextAnimation GetTextAnimationByName(string name)
        {
			return textAnimations.FirstOrDefault((t) => t.name.Equals(name));
        }

		public TextAnimation GetTextAnimation(TextType type) => textDict[type];
		...
		private  void Start()
		{
			...
			textDict = InitializeTextDictionary();
		}
